...
import {
  type ClaudeToQwenOptions,
  convertClaudeToQwen,
} from '../converters/claude-to-qwen';
...
export async function syncCodexCommands(
  ...
  for (const prompt of bundle.prompts) {
    await writeText(
      path.join(outputRoot, 'prompts', `${prompt.name}.md`),
      prompt.content + '\n',
    );
  }
  for (const skill of bundle.generatedSkills) {
    await writeText(
      path.join(outputRoot, 'skills', skill.name, 'SKILL.md'),
      skill.content + '\n',
    );
  }
}
...
export async function syncPiCommands(
  ...
  for (const extension of bundle.extensions) {
    await writeText(
      path.join(outputRoot, 'extensions', extension.name),
      extension.content + '\n',
    );
  }
}
...
export async function syncDroidCommands(
  ...
  for (const command of bundle.commands) {
    await writeText(
      path.join(outputRoot, 'commands', `${command.name}.md`),
      command.content + '\n',
    );
  }
}
...
export async function syncGeminiCommands(
  ...
  for (const command of bundle.commands) {
    await writeText(
      path.join(outputRoot, 'commands', `${command.name}.toml`),
      command.content + '\n',
    );
  }
}
...
export async function syncQwenCommands(
  ...
  for (const commandFile of bundle.commandFiles) {
    ...
    if (parts.length > 1) {
      ...
      await writeText(
        path.join(nestedDir, `${parts[parts.length - 1]}.md`),
        commandFile.content + '\n',
      );
      continue;
    }
    ...
    await writeText(
      path.join(outputRoot, 'commands', `${commandFile.name}.md`),
      commandFile.content + '\n',
    );
  }
}

export function warnUnsupportedOpenClawCommands(
  config: ClaudeHomeConfig,
): void {
  if (!hasCommands(config)) return;
...
export async function syncAntigravityCommands(
  config: ClaudeHomeConfig,
  outputRoot: string,
): Promise<void> {
  if (!hasCommands(config)) return;
  ...
  for (const workflow of bundle.workflows) {
    await writeText(
      path.join(outputRoot, 'workflows', `${workflow.name}.md`),
      workflow.content + '\n',
    );
  }
}
